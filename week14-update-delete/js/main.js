...
function buildTable(response){
  ...
  for (var i = 0; i < parsed.length; i++) {
      ...
      // Build table row
      var tr = "<tr id='" + id + "' bgcolor='" + rowColor + "'><td>" + name + "</td><td>" + dog + "</td><td>" + shots + "</td><td><a onclick='deleteUser(" + id + ")'>DELETE USER</a></td><td><a href='updateUser.php?id=" + id + "'>UPDATE USER</a></td></tr>";
...
function deleteUser(id){
  ...
  callAJAX('deleteUser.php', 'id='+id, 'deleteUserResponse');

}

function deleteUserResponse(response){
  console.log("Delete User Response:" + decodeURI(response));
  init();
}